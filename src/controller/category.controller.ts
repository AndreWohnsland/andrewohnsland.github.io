...
async function getAllDistinctCategories(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  ...
  if (mongoModel === undefined)
    return next(new AppError(`${category} not supported for categories`, 404))
  mongoModel
    .distinct('category')
    .then((cats: string[]) => res.json(cats.sort()))
    .catch((err: Error) =>
      next(
        new AppError(`Error getting categories for ${category}: ${err}`, 400),
      ),
    )
}
