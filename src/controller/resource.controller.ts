...
async function addResource(req: Request, res: Response, next: NextFunction) {
  ...
  if (reqFile === undefined || reqFile === null) return res.status(400).json({ message: 'No file given' });
  ...
  if (douplicate !== null) return res.status(409).json({ message: 'File already exists under this blog' });
...
export default { addResource, deleteResource, getAllDetails, deleteResourcesUsingBlogId };
